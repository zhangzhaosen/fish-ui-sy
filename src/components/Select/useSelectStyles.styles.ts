...
import { SelectProps, tokens, typographyStyles } from "zhangzhaohong9-fish-ui-sy";

export const selectClassNames = {
  root: "fish-ui-Select",
  select: "fish-ui-Select__select",
  icon: "fish-ui-Select__icon",
};
...
const useSelectChildStyles = makeStyles({
  base: {
    appearance: "none",
...
export const useSelectStyles = (props: SelectProps) => {
  const { size, appearance, className, disabled } = props;
  const styles = {
  ...
  const selectStyles = useSelectChildStyles();

  styles.root = mergeClasses(selectClassNames.root, rootStyles.base, className);

  styles.select = mergeClasses(
